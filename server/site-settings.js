const DEFAULT_SITE_SETTINGS = {
  ...
  team: [
    ...
    {
      name: "Dushyant Arora",
      role: "Real Estate Advisory & Client Relations",
    ...
    },
    {
      ...
      role: "Marketing, Brand & Administration",
      experience: "Around 10 years across marketing, administration and operations",
      bio: "With around 10 years of experience across marketing, administration and business operations, Neha brings together creativity, organisation and a strong understanding of business.\n\nAn MBA with a specialisation in Marketing Management, she began her professional journey in administrative and corporate roles before moving into marketing and creative communication. Her passion for art and design gives her a natural eye for visual storytelling and brand communication.\n\nAt earthsar, Neha manages marketing, creatives, social media, lead generation, HR and administration, while also playing an active role in building the company's digital and operational presence.\n\nShe was instrumental in conceptualising the earthsar name and logo, helping shape the brand identity from the very beginning.\n\nA passionate artist, Neha brings together creativity, business understanding and attention to detail to help build a distinctive and consistent earthsar brand.",
...
function cleanStats(stats = {}) {
  return {
    years: text(stats.years, 12),
    satisfaction: text(stats.satisfaction, 12),
    properties: text(stats.properties, 12)
  };
}
...
function cleanHero(hero = {}) {
  const trust = Array.isArray(hero.trust) ? hero.trust : String(hero.trust || "").split(/[|,\n]/);
  return {
    eyebrow: text(hero.eyebrow, 80),
    title: text(hero.title, 180),
    lead: text(hero.lead, 360),
    trust: trust.map(t => text(t, 40)).filter(Boolean).slice(0, 5),
    cardTitle: text(hero.cardTitle, 80),
    cardText: text(hero.cardText, 180),
    image: text(hero.image, 300),
    imageAlt: text(hero.imageAlt, 180)
  };
}
...
module.exports = { DEFAULT_SITE_SETTINGS, sanitizeSiteSettings };
